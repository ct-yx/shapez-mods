...
- 滚轮处理参考开源 [`normalizeWheel`](https://github.com/facebookarchive/fixed-data-table/blob/master/src/vendor_upstream/dom/normalizeWheel.js) 的跨设备归一化思路：标准滚轮按物理刻度切换，触控板的细小像素增量在一帧内平滑累计；只监听标准 `wheel` 事件并标记已处理事件，避免同一次滚动被重复派发后跳过变形体。按住组合键时滚轮仍阻止镜头缩放。
...
设置会由前置 mod 统一保存，其他 mod 只需要注册定义并读取 API，不需要自己编写面板或实现持久化。分类页顶端的“重载游戏”会先保存当前设置，再重载游戏渲染页面和所有本地 mod，无需关闭桌面客户端窗口；“管理模组”仍可进入游戏原生的 mod 管理页面。
