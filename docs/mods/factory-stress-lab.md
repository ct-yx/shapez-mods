...
- 支持 1×、5×、20×、任意正数倍率和 Turbo 100×；快捷键为 Shift+T 切换 Turbo，`[` / `]` 以 1× 调整手动倍率。
- Benchmark 从 200× 起步，不设置倍率上限；依据最近 FPS 窗口自适应加速或回退，目标保持在 38–42 FPS。
- 实时显示 FPS、毫秒/帧、逻辑 Tick、实体数量、模拟 Tick/s 与机器负载估算。
- 超过 10× 时限制画面展示刷新到约 1 Hz，模拟与采样继续运行。
- FPS 徽标是可点击的游戏性能详情卡：可查看实时曲线，普通详情最长记录 10 分钟。
- 可导出 2 倍分辨率 PNG、独立 HTML 和 TXT 报告，包含曲线、样本、综合分数和机器压力排名。

## 使用方法

1. 点击右上角倍率按钮打开面板；可手动输入任意正倍率。
2. 展开 PRESSURE Benchmark，选择 120 秒或 900 秒后运行；开始时面板会自动展开。
3. 运行结束后会回到 3×，再导出 PNG / WEB / TXT 报告进行布局对比。

收起普通性能详情会立即停止并清空普通曲线；Benchmark 采用独立样本记录，收起详情不会影响正在运行或已完成的压测结果。
